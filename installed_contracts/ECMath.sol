...
/* library ECMath {
  ...
  function ecadd(uint256[3] P, uint256[3] Q) private returns (uint256[3] R) {
    ...
    R[0] = addmod(addmod(mulmod(r,r,p),(p-mulmod(h1,mulmod(h1,h1,p),p)),p),(p-mulmod(2,mulmod(u1,mulmod(h1,h1,p),p),p)),p);
    R[1] = addmod(mulmod(r,addmod(mulmod(u1,mulmod(h1,h1,p),p),(p-R[0]),p),p),(p-mulmod(s1,mulmod(h1,mulmod(h1,h1,p),p),p)),p);
    R[2] = mulmod(h1,mulmod(P[2],Q[2],p),p);
  ...
  function invmod(uint256 a1, uint p1) private returns(uint256 invA) {
    uint256 t=0;
    uint256 newT=1;
    uint256 r=p1;
    uint256 newR=a1;
